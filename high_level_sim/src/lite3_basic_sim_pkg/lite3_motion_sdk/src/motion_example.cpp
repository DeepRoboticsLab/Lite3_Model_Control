 ...
 */
void MotionExample::PreStandUp(RobotCmd &cmd, double time,RobotState &data_state) {
  double standup_time = 2.50;
 ...
 * @param data_state Real-time status data of robot
 */
void MotionExample::StandUp(RobotCmd &cmd, double time,RobotState &data_state) {
  double standup_time = 2.5;
  double cycle_time = 0.001;
...
void MotionExample::SwingToAngle(Vec3 initial_angle, Vec3 final_angle,
  ...
  if(true){
    ...
    cmd.joint_cmd[3* leg_side].tor = 60 * (goal_angle[0] - data.motor_state.joint_data[3* leg_side].pos)
                                    + 0.7 * (goal_vel[0] - data.motor_state.joint_data[3* leg_side].vel);
    cmd.joint_cmd[3* leg_side+1].tor = 80 * (goal_angle[1] - data.motor_state.joint_data[3* leg_side+1].pos)
                                    + 0.7 * (goal_vel[1] - data.motor_state.joint_data[3* leg_side+1].vel);
    cmd.joint_cmd[3* leg_side+2].tor = 80 * (goal_angle[2] - data.motor_state.joint_data[3* leg_side+2].pos)
                                    + 0.7 * (goal_vel[2] - data.motor_state.joint_data[3* leg_side+2].vel);
  }
}
